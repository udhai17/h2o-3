...
public abstract class Model<M extends Model<M,P,O>, P extends Model.Parameters, O extends Model.Output> extends Lockable<M> {
  ...
  protected ModelMetrics.MetricBuilder scoreMetrics(Frame adaptFrm) {
    ...
    // Score the dataset, building the class distribution & predictions
    BigScore bs = getBigScore(domain,makeScoringNames().length,adaptFrm.means(),_output.hasWeights() && adaptFrm.find(_output.weightsName()) >= 0,computeMetrics, false /*no preds*/, null);
    bs.doAll(adaptFrm);
    return bs._mb;
  }
  ...
  public abstract class BigScore extends MRTask<BigScore> {
    ...
    public BigScore(String[] domain, int nPredCols, double[] mean, boolean testHasWeights, boolean computeMetrics, boolean makePreds, Job j) {
      _j = j;
      _domain = domain; _nPredCols = nPredCols; _mean = mean; _computeMetrics = computeMetrics; _makePreds = makePreds;
