...
def run_commands(command, number_to_run, temp_file):
    ...
    # doing a bad thing here by hardcoding the test name.
    testResult = '/Users/wendycwong/h2o-3/h2o-py/tests/results/testdir_algos_glrm_pyunit_glrm_PUBDEV_3454_NOFEATURE.py.out.txt'
    ...
    all_command = full_command+';'+list_command

    for run_index in range(0, number_to_run):
#        child = subprocess.Popen(all_command, shell=True)
        subprocess.call(all_command,shell=True)
 #       subprocess.call(list_command,shell=True)

#        while child.poll() is None:     # make sure it finishes running
 #           time.sleep(10)

        time.sleep(20)      # sleep more before grabbing test file.
            ...
            for each_line in thefile:
                ...
                if len(temp_string) > 0:
                    if "**************" in temp_string[0]:
                        ...
                        break       # done, let's quit
                    else:
                        if "###########" in temp_string[0]:
                            result_dict["iteration_number"].append(float(temp_string[-1]))
                        else:
                            if "@@@@@@@" in temp_string[0]:
...
def write_result_summary(result_dict, directory_path, is_new_run):
    ...
    if "test_name" in dict_keys:
        ...
        if "iteration_number" in dict_keys:
            if len(num_iter) > 0:
                print("iteration info is {0}.".format(num_iter))
                num_iter.extend(result_dict["iteration_number"])
            else:
                num_iter = result_dict["iteration_number"]

        if "seed" in dict_keys:
            if len(seeds) > 0:
...
def main(argv):
    ...
    print("root directory is {0}, and temp file is {1}".format(g_test_root_dir, g_temp_filename))
    if len(argv) < 3:
